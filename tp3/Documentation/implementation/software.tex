...
\end{figure}
Figure \ref{communications-diagram} shows the interaction between the various components for servicing a user request for updating the measurements. One scale unit is shown exemplary.

\subsubsection{Master Unit and User Interface}
The user interface is implemented as an HTML web page styled with CSS and all client side logic is based on the jQuery Javascript library \cite{jquery} which provides abstraction for sending remote HTTP requests and handling responses to these.
